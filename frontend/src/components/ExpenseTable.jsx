...
const ExpenseTable = () => {
  ...
  const handleCheckboxChange = async (expenseId) => {
    ...
    try {
       const res = await axios.put(`http://localhost:8080/api/v1/expense/${expenseId}/done`, {done:newStatus}, {
        headers:{
    ...
    const removeExpenseHandler = async (expenseId) => {
      try {
        const res = await axios.delete(`http://localhost:8080/api/v1/expense/remove/${expenseId}`);
        if(res.data.success){
          toast.success(res.data.message)
